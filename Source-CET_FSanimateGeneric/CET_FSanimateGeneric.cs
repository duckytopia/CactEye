//This is pretty much entirely taken from Firespitter v6.1 [for KSP 0.23]. Credit goes to Snjo.
//I've added a few lines of code for the CactEye ladder, and that's it (Rubber Ducky)
...
public class CET_FSanimateGeneric : PartModule
{
    [KSPField]
    ...
    //--------------------------------
    //I (Rubber Ducky) added these variables for my own purposes.
    //They are not from Firespitter.
    [KSPField]
    public bool CELadder = false;
    public List<Collider> ladderColList = new List<Collider>();
    ...
    public void Update()
    {
        ...
        //--------------------------------
        //This is not Firespitter code!
        if (CELadder && storedTime != animTime)
        {
            if (animTime == 0f)
            {
                foreach (Collider col in ladderColList)
                {
                    print("ENABLING LADDER COL | " + Planetarium.GetUniversalTime());
                    col.enabled = true;
                }
            }
            else
            {
                foreach (Collider col in ladderColList)
                {
                    print("DISABLING LADDER COL | " + Planetarium.GetUniversalTime());
                    col.enabled = false;
                }
            }
